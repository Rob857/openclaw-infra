...
│       ├── plugins/   # MCP adapter, Codex/Codex/Pi/qmd servers, deny rules
...
| `plugins` | plugins | MCP adapter, Codex/Codex/Pi containers, GitHub MCP, deny rules |
...
| Codex setup token | Powers OpenClaw (flat fee) | `Codex setup-token` in terminal |
...
Update secret via `pulumi config set <key> --secret`, then `pulumi up`. Tailscale key: `tailscaleAuthKey`. Codex token: `claudeSetupToken` + `openclaw auth login` on server. Gateway token: redeploy + re-pair devices. Telegram bot: revoke via @BotFather, update `telegramBotToken`, redeploy.
...
|---|---|---|
| MCP server | `github`, `codex`, `Codex` | `github-bob`, `codex-bob`, `Codex-bob` |
...
This deployment now uses a split model:
- `main` runs unsandboxed on the VPS host for operational/admin work.
- `deepseek-agent` stays sandboxed in Docker for cheaper/lower-stakes tasks.

| | `main` | `deepseek-agent` |
|---|---|
| Runtime | VPS host process context | Docker container (`openclaw-sandbox-custom:latest`) |
| Network | Host network access | Bridge (outbound internet via Docker NAT) |
| Workspace | Direct host access | Read-write (mounted at `/workspace`) |
| Host filesystem | Accessible | No access |
| Gateway config | Accessible | Isolated (can't read `~/.openclaw/`) |
| Privilege escalation | Still gated by OpenClaw tool policy | Blocked (setuid bits stripped) |
| Dev toolchain | Host-installed tools | Python 3, Node.js, git, git-lfs, ripgrep, fd, jq, yq, just, uv, pnpm, bd, sqlite3, pandoc, build-essential, ffmpeg, imagemagick, tmux, htop, tree, curl, wget, openssh-client |

**Network:** Bridge (outbound internet for web research/git push). MCP containers (Codex, Codex, Pi) use a separate `codex-proxy-net`. Sandbox containers can't reach the credential proxy.
...
agents.list[deepseek-agent].sandbox.mode: all
```
...
**Tool count:** `N_agents × Σ(tools_per_server_type)`. Per agent: github: 26, codex: 2, Codex: 2, pi: 2, qmd: 6. Check `openclaw_mcp_server_types` in `group_vars/all.yml`.
